...
[_TODO_ add explanation on the different style]{.text-danger}

#### Table of content

[_TODO_ add explanation on TOC]{.text-danger}
...
[_TODO_ stylize blockquote]{.text-danger}

Blockquote can be achieved with `markdown` using the `>` notation. To keep quote together, blank lines inside the quote must contain the `>` character. For examples
...
[_TODO_ table examples include dynamic]{.text-danger}
